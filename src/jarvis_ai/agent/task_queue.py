...
class TaskQueue:
    ...
    def submit(
        ...
        print(f"[작업큐] 📥 작업 대기열: [{task_id}] {goal[:60]}")
        return task_id
    ...
    def _run_task(self, task: Task) -> None:
        print(f"[작업큐] ▶️ 실행 중: [{task.task_id}] {task.goal[:60]}")
        try:
            ...
            if task.on_complete and not task.cancel_flag.is_set():
                try:
                    ...
                    print(f"[작업큐] ⚠️ on_complete 콜백 오류: {e}")

            print(f"[작업큐] ✅ 완료됨: [{task.task_id}]")
